...
def _check_same_pbs_repo(runtime, cc_toolchain):
    ...
    for artifact in payload:
        if artifact.owner.repo_name != runtime_repo:
            fail("Python C toolchain file {} comes from @@{}, but runtime {} comes from @@{}".format(
                artifact,
...
def _check_windows_libraries(ctx, provider_set, description, stable_abi = False):
    ...
    stem = "python3{}".format(suffix) if stable_abi else "python{}{}".format(
        ctx.attr.python_version.replace(".", ""),
        suffix,
    )
    interface_libraries = []
    dynamic_libraries = []
    for artifact in _link_library_files(provider_set):
        if artifact.extension == "lib":
            interface_libraries.append(artifact.basename)
        elif artifact.extension == "dll":
            dynamic_libraries.append(artifact.basename)

    expected_interface = ["{}.lib".format(stem)]
    expected_dynamic = ["{}.dll".format(stem)]
    if sorted(interface_libraries) != expected_interface:
        fail("{}: expected interface library {}, got {}".format(
            description,
            expected_interface,
            sorted(interface_libraries),
        ))
    if sorted(dynamic_libraries) != expected_dynamic:
        fail("{}: expected dynamic library {}, got {}".format(
            description,
            expected_dynamic,
            sorted(dynamic_libraries),
        ))
