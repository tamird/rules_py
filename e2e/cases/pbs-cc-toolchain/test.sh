...
check_toolchain 3.15 true linux_x86_64 //cases/pbs-cc-toolchain:freethreaded_315
check_toolchain 3.13 false windows_x86_64 //cases/pbs-cc-toolchain:windows_regular_313
check_toolchain 3.15 true windows_x86_64 //cases/pbs-cc-toolchain:windows_freethreaded_315
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    "$BAZEL" build \
        --lockfile_mode=off \
        --@rules_python//python/config_settings:python_version=3.8 \
        -- "$library_target"
    library="$("$BAZEL" cquery \
        ...
        --output=files \
        -- "$library_target")"
    if ! otool -L "$library" | grep -Fq '@rpath/libpython3.8.dylib'; then
        otool -L "$library" >&2
        echo "FAIL: Python 3.8 extension retained PBS's absolute dylib ID" >&2
        exit 1
    fi
fi
