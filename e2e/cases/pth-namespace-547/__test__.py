...
The exposed variant asserts the namespace is merged concretely into
site-packages for static tools. The ordinary variant asserts the same imports
compose from wheel roots without materializing that physical tree.
"""
...
def test_concrete_namespace_entries_in_site_packages():
    """Only an exposed venv materializes the namespace in site-packages.
    ...
    if os.environ.get("EXPECT_PHYSICAL_VENV") != "1":
        assert not os.path.exists(jaraco_dir), (
            "ordinary targets should import PEP 420 portions from wheel roots, "
            f"not materialize {jaraco_dir}"
        )
        return
