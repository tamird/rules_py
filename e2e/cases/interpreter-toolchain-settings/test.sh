...
if ! grep -Fq "No matching toolchains found" "$failure_log" || \
    ! grep -Fq "exec_tools_toolchain_type" "$failure_log"; then
    cat "$failure_log" >&2
    fail "Python 3.13 target incompatibility failed for an unrelated reason"
fi
