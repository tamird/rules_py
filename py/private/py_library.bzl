...
def _make_wheels_depset(ctx, extra_transitive = []):
    ...
    merged site-packages via symlinks. Postorder gives collision handling an
    explicit installation order: transitive dependencies precede their direct
    dependents, and later entries in deps and resolutions win.

    Bazel defines postorder as a left-to-right traversal of transitive children
    before direct elements:
