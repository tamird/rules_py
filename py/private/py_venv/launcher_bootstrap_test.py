...
class LauncherBootstrapTest(unittest.TestCase):
    ...
    def test_does_not_treat_stdin_after_separator_as_script(self) -> None:
        self.assertEqual(
            launcher_bootstrap._scan_args(["--", "-"]),
            launcher_bootstrap._ParsedArgs(False, False, None),
        )
    ...
    def test_resolves_relative_home_from_interpreter_dir(self) -> None:
        cwd = os.getcwd()
        with (
            mock.patch.object(
                sys,
