...
class RunfilesImportsTest(unittest.TestCase):
    ...
    def _install_manifest(self, manifest: Path, roots: list[str]) -> list[str]:
    ...
    def test_imports_namespace_package_overlay(self) -> None:
        ...
        manifest = self.root / "MANIFEST"
        manifest.write_text(
            f"_main/source/namespace_package/source.py {source / 'source.py'}\n"
            f"_main/output/namespace_package/generated.py {output / 'generated.py'}\n"
        )
        self._cleanup_module("namespace_package")
        self._install_manifest(manifest, ["_main/source", "_main/output"])
    ...
    def test_imports_package_from_manifest_directory(self) -> None:
        ...
        tree_package = importlib.import_module("tree_package")
        ...
        self.assertEqual(
            {
                (module.name, module.ispkg)
                for module in pkgutil.iter_modules(tree_package.__path__)
            },
            {("child", False)},
        )

    def test_discovers_modules_and_regular_packages(self) -> None:
        tree = self.root / "tree"
        package = tree / "discovery_package"
        namespace = tree / "discovery_namespace"
        package.mkdir(parents=True)
        namespace.mkdir()
        (package / "__init__.py").write_text("")
        (tree / "discovery_package.py").write_text("")
        (namespace / "child.py").write_text("")
        (tree / "discovery_module.py").write_text("VALUE = 'stale'\n")

        manifest_package = self.root / "manifest_package/__init__.py"
        manifest_package.parent.mkdir()
        manifest_package.write_text("")
        exact_module = self.root / "exact_module.py"
        exact_module.write_text("VALUE = 'exact'\n")
        manifest = self.root / "MANIFEST"
        manifest.write_text(
            f"_main {tree}\n"
            f"_main/discovery_manifest_package/__init__.py {manifest_package}\n"
            f"_main/discovery_module.py {exact_module}\n"
        )
        self._cleanup_module("discovery_module")
        paths = self._install_manifest(manifest, ["_main"])

        self.assertEqual(
            {
                (module.name, module.ispkg)
                for module in pkgutil.iter_modules(paths)
                if module.name.startswith("discovery_")
            },
            {
                ("discovery_manifest_package", True),
                ("discovery_module", False),
                ("discovery_package", True),
            },
        )
        self.assertEqual(importlib.import_module("discovery_module").VALUE, "exact")
