...
def assemble_venv(
    ...
    # Wheels may emit PyWheelsInfo for console scripts while leaving
    # `top_levels` empty, so their immediate layout is unknown and no root
    # entries were projected. Those wheels need `site.addsitedir` to process
    # root `.pth` files.
