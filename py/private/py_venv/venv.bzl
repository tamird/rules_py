    ...
    * **Console-script name in bin/.** Apply `package_collisions` directly
    ...
    for tl, claimants in tl_claimants.items():
        ...
        if all_namespace:
            ...
            # with an install tree so unrelated namespace portions remain
            # visible to static tools and no sibling action shares the output
            # directory. An unrelated legacy claimant can remain on .pth, but
            # a claimant involved in the conflict must be materialized.
...
def assemble_venv(
    ...
    # The PBS launcher enters the base interpreter's directory before startup.
    # CPython uses the pyvenv.cfg `home` value literally and searches upward,
    # so POSIX starts at bin while Windows starts at the install root:
    # https://github.com/aspect-build/rules_py/issues/1048
    # https://github.com/python/cpython/blob/v3.14.0/Modules/getpath.py#L210-L214
    # https://github.com/python/cpython/blob/v3.14.0/Modules/getpath.py#L367-L379
    if py_toolchain.runfiles_interpreter:
        pyvenv_home = "." if py_toolchain.python.basename == "python.exe" else "../bin"
    else:
