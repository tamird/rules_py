...
def _collect_build_tool_info(ctx):
    ...
    Each target passed via the rule's `build_tools` or `build_tool_env`
    attribute is inspected for providers:
...
def _build_action(ctx, mnemonic, progress_message):
    ...
    ctx.actions.run(
        ...
        toolchain = None,
        arguments = ctx.attr.args + patch_args + _memory_args(ctx) + path_env_args + [
            archive.path,
            wheel_dir.path,
        ],
        inputs = depset(
            [archive] + patch_inputs,
            transitive = build_tool_info.inputs,
        ),
...
`$(location)` or `$(VAR)` expansions sourced from those targets. Compiler
commands are not inferred; packages declare them and their inputs explicitly.
