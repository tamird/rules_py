        ...
        for project in mod.tags.project:
            ...
            for package in lock_data.get("package", []):
                ...
                if sdist:
                    ...
                    extra_env = {}
                    build_memory_mb = 0
                    built_wheel_metadata_known = pkg_built_wheel_metadata != None
                    ...
                    sbuild_specs[sbuild_id] = struct(
                        ...
                        built_wheel_metadata_known = built_wheel_metadata_known,
...
def _uv_impl(module_ctx):
    ...
    for sbuild_id, sbuild_cfg in cfg.sbuild_cfgs.items():
        ...
        if sbuild_cfg.built_wheel_metadata_known:
            sbuild_kwargs["built_wheel_metadata_known"] = True
...
_built_wheel_metadata_tag = tag_class(
    attrs = {
        ...
        "top_levels": attr.string_list(
            doc = "Complete, configuration-invariant list of immediate site-packages entries in wheels built from this package version's sdist. Nested namespace topology remains unknown.",
        ),
    },
