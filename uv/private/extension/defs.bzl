        ...
        for project in mod.tags.project:
            ...
            for override in mod.tags.override_package:
                ...
                if not has_target and not has_modifications:
                    fail("uv.override_package() for '{}': must specify either `target` for full replacement or at least one modification attribute (console_scripts, pre_build_patches, post_install_patches, extra_deps, extra_data, build_tool_env, build_tools, env, monitor_memory, path_env, resource_set).".format(override.name))
            ...
            for package in lock_data.get("package", []):
                ...
                if pkg_override and not sdist:
                    ...
                    if pkg_override.build_tool_env:
                        build_only_attrs.append("build_tool_env")
                    if pkg_override.build_tools:
                        build_only_attrs.append("build_tools")
                    if pkg_override.env:
                        build_only_attrs.append("env")
                    if pkg_override.monitor_memory:
                        build_only_attrs.append("monitor_memory")
                    if pkg_override.path_env:
                        build_only_attrs.append("path_env")
