...
def _whl_install(ctx):
    ...
    # Prebuilt wheel metadata is keyed by the selected wheel's basename.
    # Source-built contents are unavailable during repository evaluation, so
    # their provenance marker takes precedence over a coincidentally matching
    ...
    # Source-built wheels retain their declared scripts but have unknown
    # topology. Prebuilt wheels use metadata for the selected wheel only, so
    # inactive platform wheels cannot leak their package surface.
    # PyWheelsInfo always carries the install tree: venv assembly treats empty
    # top_levels as an unknown layout, while image layering retains the wheel
    # as a package leaf.
